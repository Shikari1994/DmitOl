...

{
  ...
  if (vids.length && !reduce && !frugal && 'IntersectionObserver' in window) {
    ...
    /* Ролики с data-hold-frame (вкладки Drill Monitor) сперва держат
       собственный первый кадр видео и только через 3 секунды продолжают
       воспроизведение с него: раньше тут был отдельный img-постер
       (скриншот), но его кадр не совпадал 1-в-1 с реальным первым кадром
       ролика — при старте воспроизведения было видно подмену картинки.
       play() → pause() сразу после загрузки заставляет браузер отрисовать
       настоящий первый кадр вместо постера/чёрного поля, дальнейший play()
       через 3 секунды продолжает ровно с этого же кадра — свап незаметен. */
