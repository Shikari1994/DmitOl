...
async function start() {
  ...
  function render(now) {
    ...
    /* Добор после подъёма: пока шар доворачивается к России (см. turn
       ...
       Отлёт (2.85) — единственная ручка размера шара в устоявшемся кадре:
       он не масштабируется, а уходит дальше за то же время, поэтому
       перспектива и скорость сцены остаются прежними. Считать так:
       камера на z = 3.2 с FOV 32°, значит дистанция в финале равна
       3.2 + отлёт, а видимый размер обратно пропорционален ей. Вертикаль
       при этом почти не едет: y здесь около нуля, а нулевую высоту
       дистанция не сдвигает. */
    const settle = smooth(clamp01((gp - RISE_END) / (0.9 - RISE_END)))
    holder.position.y = riseY + settle * fit * 0.9
    holder.position.z = -settle * fit * 2.85
