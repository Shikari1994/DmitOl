...
/* Города-акценты: список задан заказчиком — это города присутствия, а не
   выбранные «для красоты» центры добычи. Координаты реальные.

   Список НАМЕРЕННО короткий. Полный перечень городов присутствия вчетверо
   длиннее, но на шаре такого масштаба Альметьевск, Лениногорск и Бугульма
   стоят в трёх пикселях друг от друга: подписать их по отдельности
   невозможно, а сводить в общий столбик — значит превратить карту в
   легенду. Оставлена основа, по одному городу на район.

   У каждого города два указания раскладке подписей (globe/labels.js):

   side  — в какую сторону уходит линия выноски (-1 влево, +1 вправо).
           Расставлено под ФИНАЛЬНЫЙ разворот шара (SPIN_TO/TILT_TO в
           globeCanvas.js): Приобье и Восток смотрят вправо, в пустую
           середину кадра, европейская часть — влево, под заголовок,
           который к этой высоте уже кончился.
   nudge — куда сместить подпись по вертикали, в строках. Нужен только
           там, где точки стоят кучей: у четырёх городов Приобья он задаёт
           порядок выносок сверху вниз тот же, в каком стоят сами точки, —
           тогда линии идут веером и не пересекаются. Остальным ноль:
           подпись встаёт вровень со своей точкой.

   И то и другое — только ПЕРВАЯ попытка. Если место занято (заголовком
   сцены, колонкой услуг, чужой подписью или чужой точкой), раскладка сама
   переберёт сторону, длину линии и смещение.

   Порядок в списке = порядок раскладки: кто выше, тот занимает место
   первым. Поэтому вперёд вынесено Приобье — ему выбирать не из чего. */
export const CITIES = [
  // Приобье и Ямал: четыре точки в одном пятне, выноски идут веером
  { name: 'Новый Уренгой', lat: 66.084, lon: 76.68, side: 1, nudge: -1.5 },
  { name: 'Когалым', lat: 62.263, lon: 74.483, side: 1, nudge: -0.5 },
  { name: 'Сургут', lat: 61.254, lon: 73.396, side: 1, nudge: 0.5 },
  { name: 'Нижневартовск', lat: 60.939, lon: 76.57, side: 1, nudge: 1.5 },
  // Урало-Поволжье и юг
  // Fixed fan: Astrakhan remains between Oktyabrsky and Orenburg.
  { name: 'Октябрьский', lat: 54.481, lon: 53.471, side: -1, nudge: -1 },
  { name: 'Астрахань', lat: 46.348, lon: 48.033, side: -1 },
  { name: 'Оренбург', lat: 51.768, lon: 55.097, side: -1, nudge: 1 },
  /* Москва раньше Петербурга не по величине: у неё имя вдвое короче,
     места она просит меньше, а разложенный первым Петербург отобрал бы у
     неё последнее свободное поле слева. */
  { name: 'Москва', lat: 55.755, lon: 37.617, side: -1 },
  { name: 'Санкт-Петербург', lat: 59.939, lon: 30.315, side: -1 },
  // Восток
  { name: 'Усть-Кут', lat: 56.793, lon: 105.768, side: 1 },
  { name: 'Якутск', lat: 62.028, lon: 129.732, side: 1 },
  { name: 'Южно-Сахалинск', lat: 46.959, lon: 142.738, side: 1 },
]
